...
export async function POST(req: Request) {
  ...
  const tools = {
    ...
    inspectTask: tool({
      description:
        "Read a task's full plan (steps, tools, materials, safety, tips) — what's INSIDE the task, not just its title from ALL TASKS. Use this from project mode (no task open) before editing a task OR before claiming a task is unaffected by a brief/scope change. Read-only and safe to call freely. Works on ANY task; pass `task` as its # or part of its title.",
      inputSchema: z.object({
        task: z
          .string()
      ...
      execute: async ({ task }) => {
        ...
        const row = rows[0];
        if (!row)
          return { ok: false as const, message: "Task no longer exists." };
        const tk = row.task;
        const g = row.task_guide;
        return {
          ok: true as const,
          ...
          detail: tk.detail,
          status: tk.status,
          hoursEstimate: tk.hoursEstimate,
          steps: g?.steps ?? [],
          tools: g?.tools ?? [],
    ...
    setStyleProfile: tool({
      description:
        "Save the user's style choices — finishes, colors, vibe — that drive the AI dream-hero render on the project home (PHOTO_PLAN.md §5.2). Pass only the fields you've heard them name. Saving renders a fresh dream image (within a 1-minute cooldown). Hex colors must be #RGB or #RRGGBB.",
      inputSchema: z.object({
        vibe: z
          .string()
          .optional()
          .describe('Freeform overall feel, e.g. "warm scandinavian", "1920s english cottage"'),
        palette: z
          .array(z.string())
          .max(5)
          .optional()
          .describe("Up to 5 hex anchor colors, e.g. ['#F1E6D2', '#3E5C49']"),
        cabinets: z.string().optional().describe('e.g. "shaker, white"'),
        counters: z.string().optional().describe('e.g. "Carrara marble"'),
        floors: z.string().optional().describe('e.g. "natural oak herringbone"'),
        fixtures: z.string().optional().describe('e.g. "polished brass"'),
        backsplash: z.string().optional().describe('e.g. "zellige white"'),
        walls: z.string().optional().describe("Paint name or color"),
        dimensionsHint: z
          .string()
          .optional()
          .describe('e.g. "small galley", "open concept"'),
      }),
      execute: async (args) => {
        if (!writable) return denied;
        const [row] = await db
          .select({ styleProfile: projects.styleProfile })
          .from(projects)
          .where(eq(projects.id, projectId));
        const current =
          (row?.styleProfile as
            ...
            | null) ?? {};
        const finishes = { ...(current.finishes ?? {}) };
        if (args.cabinets) finishes.cabinets = args.cabinets;
        if (args.counters) finishes.counters = args.counters;
        if (args.floors) finishes.floors = args.floors;
        if (args.fixtures) finishes.fixtures = args.fixtures;
        if (args.backsplash) finishes.backsplash = args.backsplash;
        if (args.walls) finishes.walls = args.walls;
        const next: import("@/lib/style-profile").StyleProfile = {
          ...current,
          ...(args.vibe ? { vibe: args.vibe } : {}),
          ...(args.palette ? { palette: args.palette } : {}),
          ...(args.dimensionsHint
            ? { dimensionsHint: args.dimensionsHint }
            : {}),
        ...
        return commit(
          "setStyleProfile",
          async () => {
            const { updateStyleProfile } = await import("@/app/actions");
            await updateStyleProfile({ projectId, styleProfile: next });
            revalidatePath(`/p/${projectId}`);
            touch();
          },
          {
            saved: Object.keys(args).filter(
